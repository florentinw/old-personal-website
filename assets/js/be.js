;(function(g) {
          ...
          if (isFunc(f)) {
            ...
            if (r && isFunc(r.then))
              r.then(
                ...
                function(value) {
                  newPromise(false, [value])
                }
              )
            else newPromise(true, [r])
          } else newPromise(state, values)
        } catch (e) {
          newPromise(false, [e])
        }
      }
      if (state != null) setTimeout(callCallbacks, 0)
      else deferred.push(callCallbacks)
      return newPromise
    }
    return set
  }
  ...
  be.project.search = function(param, callback) {
    param = typeof param === 'object' ? param : { q: param }
    var ext = 'projects'
    return get(ext, param, callback)
  }
  ...
  hasRequire = typeof require !== 'undefined' && typeof define !== 'undefined' && isFunc(define) && isFunc(require) && isFunc(require.toUrl) && require.toUrl(basePath) === basePath

  // Check for jQuery for its .ajax() method
  hasJquery = typeof jQuery !== 'undefined' && isFunc(jQuery) && isFunc(jQuery.ajax)

  // Check for Web Worker context
  hasImport = typeof importScripts !== 'undefined' && isFunc(importScripts)
